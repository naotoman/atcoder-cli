...
def get_submit_results(contest: str, session: Session) -> Dict[str, List[str]]:
    result_url = \
        f'{ATCODER_URL}/contests/{contest}/submissions/me?orderBy=created'
...
def _get_csrf(url: str, session: Session) -> str:
    res = session.get(url)
    bs = BeautifulSoup(res.text, 'html.parser')
